...
class TapeFilter:
    ...
    def main(self, vid, table):
        ...
        if frame is not None:
            ...
            #fra3 = cv2.drawContours(pipeline.cameraInput, pipeline.find_contours_output, -1, (255,0,255),3)
            contourAreas = []
            data = []
                ...
                if(len(data) > 1 and len(slist) > 1 and len(contours) > 1):
                    for i in contours:
                        ...
                        if(cv2.contourArea(i) == slist[0] or cv2.contourArea(i) == slist[1]):
                            cv2.drawContours(pipeline.cameraInput, [i], -1, (0, 255, 0), 2)
                if(len(data) > 0 and len(slist) > 0 and len(contours) > 0):
                    for i in data:
                        if ((len(i) > 1 and len(slist) > 1) and (i[0] == slist[0] or i[0] == slist[1])):
                            newlist.append(i)
                            cv2.circle(pipeline.cameraInput, (i[1][0], i[1][1]), 7, (0, 0, 0), -1)
                            cv2.putText(pipeline.cameraInput, "center", (i[1][0] - 20, i[1][1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
...
def mainMain():
    ...
    # CAMERA 1
    cam = UsbCamera('Cam 1 Front', 0)
    cam.setResolution(160, 120)
    #cam.setExposureManual(10)f
    cam.setBrightness(60)
    cam.setFPS(60)
    print("cam1 initialized")
    # CAMERA 2
    cam2 = UsbCamera('Cam 2 Back', 1)
